...
def declare_logic():
    ...
    # Ensure that the number of passengers on a flight does not exceed the aircraft's seating capacity.
    Rule.constraint(
    validate=Flight,
    ...
    Rule.count(derive=Flight.passenger_count, as_count_of=FlightReservation)

    # Calculation of total fuel used by an aircraft based on logged entries.
    Rule.sum(derive=Aircraft.total_fuel_used, as_sum_of=FuelLog.fuel_amount)
